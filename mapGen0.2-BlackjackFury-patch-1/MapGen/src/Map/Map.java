...
public class Map {
	...
	private double minErosionWater = 2;
	private double erosionDecrement = 10;
	private double maxErosion = 150;
	private double neighbourErosionDecrement = 2;
			...
			for(int x = 0; x<gridSize; x++)
			{
				if(map[x][y].getWaterLevel()>0)
				{
					...
					if(found)
					{
						...
						if(map[targetX][targetY].getErased() < maxErosion)
						{
							double eros = map[targetX][targetY].getWaterLevel()/minErosionWater/erosionDecrement;
							map[targetX][targetY].setHeight((map[targetX][targetY].getHeight() - eros));
							map[targetX][targetY].setErased(map[targetX][targetY].getErased() + eros);
							
							newX = (targetX-1)%gridSize;
							if(newX<0)
								newX+=gridSize;
							newY = targetY;
							if(map[newX][newY].getErased() < maxErosion/neighbourErosionDecrement)
							{
								map[newX][newY].setHeight((map[newX][newY].getHeight() - eros/neighbourErosionDecrement));
								map[newX][newY].setErased(map[newX][newY].getErased() + eros/neighbourErosionDecrement);
								System.out.println(1);
							}
